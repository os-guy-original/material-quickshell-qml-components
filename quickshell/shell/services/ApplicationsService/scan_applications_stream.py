...
Outputs each app as it's found for instant UI updates
"""
...
def main():
    """Scan and stream applications as newline-delimited JSON"""
    seen = set()
    ...
    for search_dir in get_search_paths():
        try:
            for desktop_file in search_dir.glob('*.desktop'):
                ...
                if app_data:
                    # Output each app immediately as JSON line
                    print(json.dumps(app_data, ensure_ascii=False))
                    sys.stdout.flush()
        except Exception:
            continue
